...
if    info['role'] == 'supporter'  :                       #if qr_cam detected qr_data as supporter
    ...
    if AI_cam['status']  :
        print("working")#AI_cam detecting plastic
...
elif  info['role'] == 'collector' :                        #else if its a collector then athentify him from server and get a boolean
    if Collector_Authentif (collector):                  #if authentified command solenoid to give him access to repository
