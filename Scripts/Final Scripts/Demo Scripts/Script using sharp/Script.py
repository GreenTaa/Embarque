...
def Get_Status ():                                                             #this function establish a serial cmmunication between RPI and the PCB
        ...
        print(line)
        status = {"id_poubelle": "615317ecb94fd7811415cd52", "prs" : line }
...
def Receive_Input_State () :
    #if __name__=='__main__':
    #    ser=serial.Serial('/dev/ttyACM0',115200,timeout=1)                    # initialize and configure serial port
        ser.flush()
        ser.write(b"5\n")                                                       # clear any existing data
        stop =0                                                             #counting variable
        while stop <= 2:
            #ser.write(b"5\n")
            ser.flush()                                                  # 3 iterations needed                                               # 5 is the command to activate proximity sensor, send 5 to pcb
            line=ser.readline().decode('utf-8').rstrip()                    #get response from PCB
            print(line)
            #sleep(1)
            stop+=1                                                         #increment by one
        ...
        while received_state == "Exist" :
            ...
            if Type == 'plastic'  :
                ...
                nb_bottles+=1                                    #increment nb bottles by 1
                    #activate_qr=True                                        #This sleep is a waiting period and its important
            else :
